                                    ...
                                    <?php foreach ($documents as $k => $row) : ?>
                                        <div class="row mb-3 ">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <button type="button" onclick="delete_modal('<?= base_url() ?>Delete-Project-Document/<?= $obj->ProjectId ?>/<?= $row->DocId ?>');" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                                            </div>
                                        </div>
                                        <div class="append-<?= $row->DocId ?> mb-5"></div>
                                    <?php endforeach; ?>
                                    <form action="<?= base_url() ?>admin/Project/docform/<?= $obj->ProjectId ?>" method="post" enctype="multipart/form-data">
                                        <div class="row">
...
<script>
    function edit_row(DocId) {
        ...
        var solution_id = <?= $obj->ProjectId ?>;
        $("."+app_btn).hide();
        $.ajax({
            type: 'ajax',
            method: 'POST',
            url: "<?= base_url(); ?>admin/Project/get_document",
            data: {
                DocId: DocId,
            },
            dataType: 'json',
            success: function(data) {
                var html = "";
                $("."+app_div).empty();
                html += "<form action=\"<?= base_url() ?>admin/Project/editdoc\/" + data['details'].DocId +"/" + solution_id + "\"  method=\"post\" enctype=\"multipart/form-data\">";
                html +="<div class=\"row\">";

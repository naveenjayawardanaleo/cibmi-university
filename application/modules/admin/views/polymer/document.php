                                    ...
                                    <?php foreach ($documents as $k => $row) : ?>
                                        <div class="row mb-3 ">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <button type="button" onclick="delete_modal('<?= base_url() ?>Delete-Polymer-Document/<?= $obj->PolymerId ?>/<?= $row->DocId ?>');" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                                            </div>
                                        </div>
                                        <div class="append-<?= $row->DocId ?> mb-5"></div>
                                    <?php endforeach; ?>
                                    <form action="<?= base_url() ?>admin/Polymer/docform/<?= $obj->PolymerId ?>" method="post" enctype="multipart/form-data">
                                        <div class="row">
...
<script>
    function edit_row(DocId) {
        ...
        var solution_id = <?= $obj->PolymerId ?>;
        $("."+app_btn).hide();
        $.ajax({
            type: 'ajax',
            method: 'POST',
            url: "<?= base_url(); ?>admin/Polymer/get_document",
            data: {
                DocId: DocId,
            },
            dataType: 'json',
            success: function(data) {
                var html = "";
                $("."+app_div).empty();
                html += "<form action=\"<?= base_url() ?>admin/Polymer/editdoc\/" + data['details'].DocId +"/" + solution_id + "\"  method=\"post\" enctype=\"multipart/form-data\">";
                html +="<div class=\"row\">";

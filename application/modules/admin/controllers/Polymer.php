...
class Polymer extends MY_Controller
{
    var $page = "polymer";
    var $img_width = '700px';
    ...
    {
        parent::__construct();
        $this->load->model('Polymer_model', 'model');
        $this->load->model('Polymer_document_model', 'document');
    ...
    public function delete($id)
    {
        ...
        if ($res) {
            $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Polymer Delete Successfully </strong>']);
        } else {
            $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failure !!! </strong>']);
        }
        redirect(base_url() . 'Manage-Polymer');
    }
    ...
    public function form($id = 0)
    {
        ...
        if ($id) {
            ...
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Polymer Updated Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Edit-Polymer/' . $id);
        } else {
            $res = $this->model->insert($post);
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Polymer Created Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Create-Polymer');
        }
    }
    ...
    public function documents_upload($id)
    {
        if ($this->session->has_userdata('user')) {
            $data['obj'] = $this->model->get($id);
            $data['documents'] = $this->document->get_by(array('PolymerId' => $id));
    ...
    public function docform($id)
    {
        ...
        redirect(base_url() . 'Upload-Polymer-Documents/' . $id);
    }
    ...
    public function editdoc($id,$iid)
    {
        ...
        redirect(base_url() . 'Upload-Polymer-Documents/' . $iid);

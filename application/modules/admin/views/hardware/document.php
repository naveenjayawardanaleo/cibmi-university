                                    ...
                                    <?php foreach ($documents as $k => $row) : ?>
                                        <div class="row mb-3 ">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <button type="button" onclick="delete_modal('<?= base_url() ?>Delete-Hardware-Document/<?= $obj->HardwareId ?>/<?= $row->DocId ?>');" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                                            </div>
                                        </div>
                                        <div class="append-<?= $row->DocId ?> mb-5"></div>
                                    <?php endforeach; ?>
                                    <form action="<?= base_url() ?>admin/Hardware/docform/<?= $obj->HardwareId ?>" method="post" enctype="multipart/form-data">
                                        <div class="row">
...
<script>
    function edit_row(DocId) {
        ...
        var solution_id = <?= $obj->HardwareId ?>;
        $("."+app_btn).hide();
        $.ajax({
            type: 'ajax',
            method: 'POST',
            url: "<?= base_url(); ?>admin/Hardware/get_document",
            data: {
                DocId: DocId,
            },
            dataType: 'json',
            success: function(data) {
                var html = "";
                $("."+app_div).empty();
                html += "<form action=\"<?= base_url() ?>admin/Hardware/editdoc\/" + data['details'].DocId +"/" + solution_id + "\"  method=\"post\" enctype=\"multipart/form-data\">";
                html +="<div class=\"row\">";

...
class Project extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Project_document_model', 'document');
    ...
    public function delete($id)
    {
        ...
        if ($res) {
            $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Project Delete Successfully </strong>']);
        } else {
            $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failure !!! </strong>']);
        }
        redirect(base_url() . 'Manage-Project');
    }
    ...
    public function form($id = 0)
    {
        ...
        if ($id) {
            ...
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Project Updated Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Edit-Project/' . $id);
        } else {
            $res = $this->model->insert($post);
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Project Created Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Create-Project');
        }
    }
    ...
    public function documents_upload($id)
    {
        if ($this->session->has_userdata('user')) {
            $data['obj'] = $this->model->get($id);
            $data['documents'] = $this->document->get_by(array('ProjectId' => $id));
    ...
    public function docform($id)
    {
        ...
        redirect(base_url() . 'Upload-Project-Documents/' . $id);
    }
    ...
    public function editdoc($id,$iid)
    {
        ...
        redirect(base_url() . 'Upload-Project-Documents/' . $iid);

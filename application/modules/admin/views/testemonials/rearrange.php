        ...
        <div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            <div class="d-flex align-items-center flex-wrap mr-2">
                <h5 class="text-dark font-weight-bold mt-2 mb-2 mr-5">
                    Dashboard > Rearrange Testemonials
                </h5>
            </div>
        </div>
    </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12">
                                    <div class="card-box">
                                        ...
                                        <input type="hidden" id="view" value="testemonials" />
                                        <input type="hidden" id="id" value="TestemonialsId" />
                                        <input type="hidden" id="action" value="<?= base_url('admin/reorder/do_update') ?>" />

                                        <div class="row" id="sortable">
                                            <?php if (is_array($records)) :
                                                foreach ($records as $k => $row) : ?>
                                                    <div class="col-lg-6 col-sm-6 col-xs-6 col-md-6 col-xl-3" role="alert" style="cursor: move" id="conid_<?= $row->TestemonialsId ?>">
                                                        <div class="card card-custom">
                                                            <!-- <img src="<?= UP . $row->Image ?>" alt="<?= $row->Image ?>"> -->
                                                        </div>
                                                        <div class="text-dark-75 font-weight-bolder font-size-h6 text-center"><?= $row->Order ?> <?= $row->Name ?><br><?=$row->Designation?></div>
                                                    </div>
                                            <?php endforeach;
                                            endif; ?>
                                        </div>

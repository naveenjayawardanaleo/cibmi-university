...
class Pantry extends MY_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('Pantry_model', 'model');
        $this->load->model('Pantry_document_model', 'document');
    ...
    public function delete($id)
    {
        ...
        if ($res) {
            $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Pantry Delete Successfully </strong>']);
        } else {
            $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failure !!! </strong>']);
        }
        redirect(base_url() . 'Manage-Pantry');
    }
    ...
    public function form($id = 0)
    {
        ...
        if ($id) {
            ...
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Pantry Updated Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Edit-Pantry/' . $id);
        } else {
            $res = $this->model->insert($post);
            if ($res) {
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Pantry Created Successfully </strong>']);
            } else {
                $this->session->set_flashdata('notification', ["alert" => "danger", "text" => '<strong> Update Failed </strong>']);
            }
            redirect(base_url() . 'Create-Pantry');
        }
    }
    ...
    public function documents_upload($id)
    {
        if ($this->session->has_userdata('user')) {
            $data['obj'] = $this->model->get($id);
            $data['documents'] = $this->document->get_by(array('PantryId' => $id));
    ...
    public function docform($id)
    {
        ...
        redirect(base_url() . 'Upload-Pantry-Documents/' . $id);
    }
    ...
    public function editdoc($id,$iid)
    {
        ...
        redirect(base_url() . 'Upload-Pantry-Documents/' . $iid);
